...
test("under-lock fenced read rejects expired and foreign lock capabilities", async () => {
  await withProject(async (projectDir) => {
    ...
    try {
      ...
      await withLock(otherProject, async (foreignContext) => {
        await assert.rejects(readFencedStateUnderLock(foreignContext, { projectDir }), { code: "CLIMIER_INVALID_LOCK_CONTEXT" });
      });
    } finally {
...
test("under-lock fenced read returns null without creating paths for an absent project", async () => {
  await withProject(async (projectDir) => {
    const statePath = stateFile(projectDir);
    const ledgerPath = ledgerFile(projectDir);
    const result = await withLock(projectDir, (lockContext) => readFencedStateUnderLock(lockContext));
...
test("under-lock fenced read rejects v5 state without its ledger", async () => {
  await withProject(async (projectDir) => {
    const { file } = await seedState(projectDir);
    await bootstrapFencedState(projectDir);
    await fs.unlink(ledgerFile(projectDir));
    await assert.rejects(
      withLock(projectDir, (lockContext) => readFencedStateUnderLock(lockContext)),
...
test("under-lock fenced read fails closed for ledger-only projects unless bootstrap is pending", async () => {
  await withProject(async (projectDir) => {
    const ledgerPath = ledgerFile(projectDir);
...
test("legacy update racing under-lock migration is fenced after migration publishes", async () => {
  await withProject(async (projectDir) => {
    const { file } = await seedState(projectDir);
    ...
    assert.equal(migrated.log.some((entry) => entry.action === "racing-legacy-write"), false);
  });
});

test("under-lock fenced read migrates existing legacy state with exact fingerprints", async () => {
  await withProject(async (projectDir) => {
    const { file, raw: sourceRaw } = await seedState(projectDir);
    const state = await withLock(projectDir, (lockContext) => readFencedStateUnderLock(lockContext));
...
test("under-lock read recovers only the exact durable migration pending record", async (t) => {
  for (const faultAt of ["after-pending", "after-state-rename"]) {
    await t.test(faultAt, async () => withProject(async (projectDir) => {
      ...
      const recovered = await withLock(projectDir, (lockContext) => readFencedStateUnderLock(lockContext));
