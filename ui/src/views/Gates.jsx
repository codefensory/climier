  ...
  return (
    <div class="flex h-full flex-col">
      <div class="flex flex-wrap items-center gap-3 border-b border-line px-4 py-2">
        ...
        <select class="rounded border border-line bg-panel px-2 py-1 text-xs text-slate-300 outline-none" value={purpose()} onChange={(e) => setPurpose(e.currentTarget.value)}>
      ...
      <div class="flex-1 space-y-2 overflow-auto p-4">
        <Show when={list().length} fallback={<Empty>No gates match.</Empty>}>
          <For each={list()}>
            {(g) => (
              <button class="w-full rounded-lg border border-line bg-panel p-3 text-left hover:border-amber-500/40" onClick={() => select(g.id)}>
                <div class="flex flex-wrap items-center gap-2">
                  <span class="mono text-xs text-amber-300">{g.id}</span>
                  <StatusBadge status={g.status || "open"} />
                  <Chip>{g.purpose || "decision"}</Chip>
                  <Show when={g.initiative}><Chip><span class="text-sky-300">{g.initiative}</span></Chip></Show>
                  <span class="ml-auto text-[11px] text-slate-500">notes {g.notes?.length || 0}</span>
                </div>
                <div class="mt-1.5 text-sm font-medium text-slate-100">{g.title}</div>
                <Show when={g.resolution}>
                  <div class="mt-1 text-xs text-teal-300">
