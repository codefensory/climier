...
function EqCommand(props) {
  ...
  return <Show when={cmd}><div class="mono rounded-lg bg-mid px-2 py-1.5 text-[11px] text-emerald-800">{cmd}</div></Show>;
}
...
export default function NodeDetail() {
  ...
  return (
    <Show when={selectedId()}>
      ...
      <aside class="fixed right-0 top-0 z-30 flex h-full w-full max-w-2xl flex-col border-l border-line bg-canvas">
        <div class="flex items-center gap-2 border-b border-line px-4 py-3">
          <button class="rounded-full border border-line bg-panel-2 px-2 py-0.5 text-xs text-slate-600 hover:text-ink" onClick={() => select(null)}>✕</button>
          <span class="mono text-sm text-sky-700">{selectedId()}</span>
        ...
        <div class="flex-1 space-y-4 overflow-auto p-4">
          <Show when={detailError()} fallback={<Show when={d()} fallback={<div class="text-sm text-slate-500">Loading…</div>}>
            <div>
              <h2 class="text-lg font-semibold leading-snug text-slate-900">{d().node.title}</h2>
              <div class="mt-2 flex flex-wrap items-center gap-2">
                ...
                <Show when={d().node.initiative}><Chip><span class="text-sky-700">{d().node.initiative}</span></Chip></Show>
                <Show when={d().node.domain}><Chip>{d().node.domain}</Chip></Show>
                <For each={d().node.tags || []}>{(t) => <Chip>#{t}</Chip>}</For>
              </div>
              <Show when={d().node.claim}>
                <div class="mt-2 text-xs text-sky-700">claimed by <span class="mono">{d().node.claim.by}</span> at {fmtTime(d().node.claim.ts)}</div>
              </Show>
              <Show when={EXPLAIN[d().derived_status]}>
                <div class="mt-2 rounded-lg border border-line bg-panel-2 p-2 text-xs text-slate-600">{EXPLAIN[d().derived_status]}</div>
              </Show>
            ...
            <Section title="Specification">
              <Show when={d().node.body} fallback={<div class="text-xs text-slate-500">No body.</div>}>
                <div class="whitespace-pre-wrap text-sm text-slate-700">{d().node.body}</div>
              </Show>
              <Show when={d().node.definition}>
                <div class="mt-3">
                  <div class="text-xs font-semibold uppercase tracking-wider text-slate-500">Definition</div>
                  <div class="mt-1 whitespace-pre-wrap text-sm text-slate-700">{d().node.definition}</div>
                </div>
              </Show>
              <Show when={d().node.acceptance}>
                <div class="mt-3">
                  <div class="text-xs font-semibold uppercase tracking-wider text-slate-500">Acceptance</div>
                  <div class="mt-1 whitespace-pre-wrap text-sm text-emerald-800">{d().node.acceptance}</div>
                </div>
              </Show>
              <Show when={d().node.resolution}>
                <div class="mt-3 rounded-lg border border-teal-600/30 bg-teal-500/10 p-2">
                  <div class="text-xs font-semibold uppercase tracking-wider text-teal-700">Resolution · {d().node.resolution.choice}</div>
                  <div class="mt-1 text-sm text-slate-700">{d().node.resolution.rationale}</div>
                </div>
              </Show>
            </Section>
                  ...
                  <For each={d().blocking}>
                    {(b) => (
                      <button class={`flex w-full items-center gap-2 rounded border p-2 text-left ${b.satisfied ? "border-line opacity-60" : "border-rose-500/40 bg-rose-500/5"}`} onClick={() => select(b.node.id)}>
                        <span class={`h-2 w-2 shrink-0 rounded-full ${b.satisfied ? "bg-emerald-500" : "bg-rose-500"}`} />
                        <span class="mono text-xs text-sky-700">{b.node.id}</span>
                        <span class="truncate text-xs text-slate-600">{b.node.title}</span>
              ...
              <Show when={d().dependents.length} fallback={<div class="text-xs text-slate-500">No dependents.</div>}>
                <div class="space-y-1.5">
                  <For each={d().dependents}>
                    {(dp) => (
                      <button class="flex w-full items-center gap-2 rounded-lg border border-line p-2 text-left hover:border-sky-600/50" onClick={() => select(dp.node.id)}>
                        <span class="mono text-xs text-sky-700">{dp.node.id}</span>
                        <span class="truncate text-xs text-slate-600">{dp.node.title}</span>
              ...
              <Show when={d().knowledge.length} fallback={<div class="text-xs text-slate-500">No scoped knowledge applies.</div>}>
                <div class="space-y-1.5">
                  <For each={d().knowledge}>
                    {(k) => (
                      <button class={`w-full rounded-lg border p-2 text-left ${k.status === "deprecated" ? "border-line opacity-50" : "border-violet-600/30"}`} onClick={() => select(k.id)}>
                        <div class="flex items-center gap-2">
                          <span class="mono text-xs text-violet-700">{k.id}</span>
                          <span class="text-xs font-medium text-slate-800">{k.title}</span>
                        ...
                        <Show when={k.mitigation}><div class="mt-0.5 text-[11px] text-teal-700">mitigation: {k.mitigation}</div></Show>
                      </button>
                    )}
                  </For>
                </div>
                    ...
                    {(note) => (
                      <div class="rounded border border-line bg-panel-2 p-2">
                        <div class="flex items-center gap-2 text-[11px] text-slate-500">
                          <span class="mono text-sky-700">{note.agent}</span>
                          <span>{fmtTime(note.ts)}</span>
                        </div>
                        <div class="mt-1 whitespace-pre-wrap text-xs text-slate-700">{note.text}</div>
                      </div>
                    )}
                  </For>
                </div>
                ...
                <For each={[...(d().history || [])].reverse()}>
                  {(h) => (
                    <div class="flex items-center gap-2 text-xs">
                      ...
                      <span class="mono rounded-full bg-panel-2 px-1 text-[11px] text-sky-700">{h.action}</span>
                      <span class="mono text-slate-500">{h.agent}</span>
            ...
            <Section title="Refs">
              <Show when={d().refs.length} fallback={<div class="text-xs text-slate-500">No structured refs or detected markdown references.</div>}>
                <div class="space-y-1">
                  <For each={d().refs}>
                    {(r) => <div class="mono truncate text-xs text-sky-700" title={r}>{r}</div>}
                  </For>
                </div>
              </Show>
            </Section>
          </Show>}>
            <div class="text-sm text-rose-700">{detailError()}</div>
          </Show>
        </div>
      </aside>
    </Show>
