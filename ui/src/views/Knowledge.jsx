    ...
    <div class="flex h-full flex-col">
      ...
      <div class="flex-1 space-y-2 overflow-auto p-4">
        <Show when={list().length} fallback={<Empty>No knowledge nodes match.</Empty>}>
          <For each={list()}>
            {(k) => (
              <button class="w-full rounded-lg border border-violet-500/25 bg-panel p-3 text-left hover:border-violet-500/50" onClick={() => select(k.id)}>
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <Show when={k.initiative}><Chip><span class="text-sky-300">{k.initiative}</span></Chip></Show>
                </div>
                <div class="mt-1.5 text-sm font-medium text-slate-100">{k.title}</div>
                ...
                <Show when={scopeChips(k).length}>
                  <div class="mt-2 flex flex-wrap gap-1">
                    <For each={scopeChips(k)}>
                      {([label, kind]) => (
                        <span class={`rounded px-1.5 py-0.5 text-[10px] border ${kind === "node" ? "border-rose-500/30 text-rose-300" : "border-line text-slate-400"}`}>
                          {kind}: {label}
                        </span>
                      )}
                    </For>
                ...
                <Show when={k.mitigation}>
                  <div class="mt-1 text-[11px] text-teal-300">mitigation: {k.mitigation}</div>
                </Show>
              </button>
            )}
          </For>
