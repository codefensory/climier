...
- `NOT_OWNER`
- `INVALID_STATUS`
- `UNSUPPORTED_V2_FIELD` — a v1-only field (e.g. `--skills`, `--effort`, `--priority`) was passed on a v2 state; v2 has no such fields, use `--body`/`--meta`
- `V1_ONLY_ON_V2_STATE` — a v1-only command (`claim`, `done`, `block`, `decide`, `promote`, `archive`, `ready`, `pre-claim`, `next`, `tasks`, `graph`, `gotchas`, `decisions`, `next-id`, `add-decision`, `add-gotcha`, `close-gotcha`, `reopen-gotcha`) was run on a v2 state; use the v2 surface (`take`/`resolve`/`context`/`add-gate`/`add-knowledge`/...) instead
