...
- **Plan location**: this execution plan lives in `docs/plans/`, the repository's
  sole versioned directory for plans. See ADR-023.
...
- **Resolved by ADR-023:** all versioned plans live in `docs/plans/`; there is no
  separate `.plans/` directory. The docs site publishes this directory through its
